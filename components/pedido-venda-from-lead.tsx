...
import { Plus, Save, Search, ChevronDown, ShoppingCart, Package, X, Edit, TrendingUp, Percent, DollarSign, Info } from "lucide-react"
...
export default function PedidoVendaFromLead({
  ...
  const [pedido, setPedido] = useState(() => {
    ...
    let codEmpInicial = dadosIniciais.CODEMP || "";
    try {
      const userStr = document.cookie
        .split('; ')
        .find(row => row.startsWith('user='))
      ...
      if (userStr) {
        const user = JSON.parse(decodeURIComponent(userStr));
        const emp = user.CODEMP || user.codEmp || user.cod_emp;
        if (emp) {
          codEmpInicial = String(emp);
          console.log('✅ CODEMP inicial obtido do cookie:', codEmpInicial);
        }
      }
    } catch (e) {
  ...
  const handleAdicionarItemCarrinho = (produto: any, quantidade: number, desconto?: number, controle?: string, localEstoque?: number, maxDesconto?: number, maxAcrescimo?: number, precoBase?: number, politicaAplicada?: any) => {
    ...
    const novoItem = {
      ...
      preco: vlrUnitario,
      MAX_DESC_PERMITIDO: produto.MAX_DESC_PERMITIDO || produto.RESULT_PERCDESCONTO_MAX || maxDesconto, // Using passed maxDesconto if available
      MAX_ACRE_PERMITIDO: produto.MAX_ACRE_PERMITIDO || produto.RESULT_PERCACIMA_MAX || maxAcrescimo,     // Using passed maxAcrescimo if available
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-white">
                        ...
                        <div className="overflow-x-auto">
                          <table className="w-full text-[10px] md:text-xs">
                            ...
                            <tbody className="divide-y divide-gray-100">
                              {itens.map((item, index) => {
                                const itemImposto = impostosItens.find(imp => String(imp.codigoProduto) === String(item.CODPROD))
                                const totalImpostos = itemImposto?.impostos?.reduce((sum: number, imp: any) => sum + (imp.valorImposto || 0), 0) || 0
                                    ...
                                    <td className="p-2 py-3">
                                      ...
                                      {itemImposto?.impostos && itemImposto.impostos.length > 0 && (
                                        <div className="flex flex-wrap gap-1 mt-1 opacity-70">
                                          {itemImposto.impostos.map((imp: any, idx: number) => (
                                            <span key={idx} className="text-[9px] bg-blue-50 text-blue-600 px-1 rounded uppercase">
                                              {imp.tipo}: {formatCurrency(imp.valorImposto)}
                                            </span>
                                          ))}
                                        </div>
                                      )}
                                    </td>
                                    <td className="p-2 text-center text-gray-600">{item.QTDNEG}</td>
                                    <td className="p-2 text-right text-gray-600">{formatCurrency(item.VLRUNIT)}</td>
                                    <td className="p-2 text-center text-orange-600 font-medium">{item.PERCDESC || 0}%</td>
                                    <td className="p-2 text-right text-blue-600 font-medium">{formatCurrency(totalImpostos)}</td>
                                    <td className="p-2 text-right font-bold text-green-700">{formatCurrency(calcularTotal(item))}</td>
                                  </tr>
                                )
                              })}
                            </tbody>
                            <tfoot className="bg-gray-50 font-bold">
                              <tr>
                                <td colSpan={5} className="p-2 text-right text-gray-700 uppercase tracking-wider text-[10px]">TOTAL DO PEDIDO:</td>
                                <td className="p-2 text-right text-green-700 text-sm md:text-base">
                                  {formatCurrency(calcularTotalPedido())}
                                </td>
                              </tr>
                            </tfoot>
                          ...
                          {impostosItens.length > 0 && (
                            <div className="mt-3 p-3 bg-blue-50/50 rounded-lg border border-blue-100">
                              <div className="flex justify-between items-center text-blue-800">
                                <span className="text-xs font-bold uppercase tracking-wider">Total de Impostos (Simulado):</span>
                        ...
                        onClick={() => {
                          try {
                            ...
                            const tableData = itens.map(item => {
                              ...
                              return [
                                itemImposto?.DESCRPROD || item.DESCRPROD || '-',
                            ...
                            autoTable(doc, {
                              head: [['Produto', 'Qtd', 'Vlr. Unit.', 'Desc %', 'Impostos', 'Total']],
                              body: tableData,
                              startY: y,
                              theme: 'grid',
                              ...
                              foot: [[
                                { content: 'TOTAL DO PEDIDO:', colSpan: 5, styles: { halign: 'right', fontStyle: 'bold' } },
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-md border-t border-[#F2F2F2] p-3 md:p-4 flex justify-between items-center z-[110] shadow-[0_-4px_20px_-10px_rgba(0,0,0,0.1)] pointer-events-none">
        <div className="flex gap-4 md:gap-8 ml-2 md:ml-4 pointer-events-auto">
        ...
        <div className="flex gap-3 mr-2 md:mr-4 pointer-events-auto">
          <Button
            variant="outline"
            onClick={onCancel}
